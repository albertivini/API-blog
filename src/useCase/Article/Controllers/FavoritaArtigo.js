        ...
        try {
            ...
            if (artigo != false) {
                ...
                if (confere == false) {
                    ...
                    await Favorite.favoritaArtigo(artigo.id, id)
                    ...
                    return res.status(401).json({ success: false, message: "Postagem já favoritada."})
                }
            } else {
                return res.status(404).json({ success: false, message: "Postagem não foi encontrada."})
        }
        } catch (err) {
            return res.status(500).json({ success: false, message: "Não foi possível favoritar a postagem."})
        }
    }
}